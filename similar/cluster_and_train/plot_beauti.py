...
df_filtered = df_filtered[df_filtered['KL_Similarity'] < 0.35]
...
# --- 5. 视觉逻辑润色 ---
ax.axvspan(df_filtered['KL_Similarity'].min(), boundary_x,
...
ax.text(boundary_x - 0.01, ax.get_ylim()[1] * 0.92,
        'In-Distribution',
        ha='right', fontsize=16, color='gray',
...
if y_ours_end < y_baseline_best_end:
    ax.annotate(
        '',
        xy=(max_x, y_ours_end),
        xytext=(max_x, y_baseline_best_end),
        arrowprops=dict(arrowstyle='<->', color='black', lw=1.5)
    )
    ax.text(
        max_x + 0.005,
        (y_ours_end + y_baseline_best_end) / 2,
        'Robustness\nGap',
        va='center',
        fontweight='bold',
        color=color_map[our_method_name],
        fontsize=14
    )
...
# --- 6. 细节修饰 ---
ax.set_xlabel('Distribution Shift (MMD)', labelpad=12)
...
order = [labels.index(ours_label)] + [
    i for i, l in enumerate(labels) if l != ours_label
]
...
# --- 7. 保存图片 ---
plt.savefig("Generalization_Results.pdf", bbox_inches='tight', dpi=300)
plt.savefig("Generalization_Results.png", bbox_inches='tight', dpi=300)

print("Success! Figures saved.")
plt.show()
