...
# ==================== 数据处理 ====================
df = pd.read_csv(os.path.join("script", "modified_methods.csv"))
...
def create_double_bar_plot_with_spaced_legend(dataframe, file_prefix, figsize=(18, 7)):
    ...
    ax1.set_xlabel('Training Data Ratio(%)', fontsize=22, weight='bold', labelpad=10)
    ax1.set_title('Distribution Distance to Original Data (MMD)', fontsize=21, weight='bold', pad=10)
    ...
    ax2.set_xlabel('Training Data Ratio(%)', fontsize=22, weight='bold', labelpad=10)
    ax2.set_title('Distribution Distance to Test Data (MMD)', fontsize=21, weight='bold', pad=10)
...
create_double_bar_plot_with_spaced_legend(
    dataframe=df,
    file_prefix='final_mmd_comparison_nature_style_en'
)
