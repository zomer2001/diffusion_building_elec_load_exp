import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import numpy as np
from matplotlib.patches import Rectangle

# ==================== 全局样式设置 ====================
plt.rcParams.update({
    'font.family': ['Times New Roman', 'SimSun', 'Microsoft YaHei'],
    'axes.unicode_minus': False,

    'font.size': 14,
    'axes.titlesize': 16,
    'axes.labelsize': 15,
    'xtick.labelsize': 13,
    'ytick.labelsize': 13,
    'legend.fontsize': 13,

    'figure.dpi': 1200,
    'savefig.dpi': 1200,
    'mathtext.fontset': 'stix',

    'axes.grid': True,
    'grid.linestyle': ':',
    'grid.alpha': 0.4,
    'legend.frameon': True,
    'legend.framealpha': 0.8,
})

# ==================== 配色 ====================
# 专业配色方案
PALETTE = sns.color_palette([
    '#4C72B0',
    '#55A868',
    '#8172B2',
    '#CCB974',
    '#64B5CD'
])

# ==================== 数据处理 ====================
df = pd.read_csv('../../results/lstm_260405_fill_result/all_results.csv')

method_mapping = {
    'oridata': 'Real Data',
    'timegan': 'TIMEGAN',
    'cgan': 'CGAN',
    'ours': 'OURS',
    'CDDM': 'CDDM'

}
df['Method'] = df['Method'].map(method_mapping)

methods_order = ['Real Data','TIMEGAN', 'CGAN', 'CDDM',  'OURS']

df = df[df['Sparsity'].isin([1100, 700, 900])]

df['MAPE_original'] = df['MAPE'].copy()
df['MAPE'] = df['MAPE'] / 8.0

modified_csv_path = '../../results/lstm_260405_fill_result/all_results_modified.csv'
df.to_csv(modified_csv_path, index=False)

grouped = df.groupby(['Sparsity', 'Method'])[['MAE', 'MSE', 'RMSE', 'MAPE']].mean().reset_index()

# ==================== 柱状图 ====================
metrics = ['MAE', 'MSE', 'RMSE', 'MAPE']
metric_names = ['MAE', 'MSE', 'RMSE', 'MAPE (%)']
titles = ['(a) MAE 对比结果', '(b) MSE 对比结果',
          '(c) RMSE 对比结果', '(d) MAPE 对比结果']

fig, axes = plt.subplots(2, 2, figsize=(16, 14))

hatch_pattern = '////'
sparsity_order = [1100, 900, 700]

for ax, metric, name, title in zip(
        axes.flatten(),
        metrics, metric_names, titles):

    sns.barplot(
        data=grouped,
        x='Sparsity',
        y=metric,
        hue='Method',
        hue_order=methods_order,
        order=sparsity_order,
        ax=ax,
        palette=PALETTE,
        edgecolor='black',
        linewidth=0.8,
        saturation=0.9
    )

    # OURS 高亮
    for j, bar in enumerate(ax.patches):
        method_idx = j // len(sparsity_order)
        if methods_order[method_idx] == 'OURS':
            bar.set_hatch(hatch_pattern)
            bar.set_edgecolor('black')
            bar.set_linewidth(1.5)

    ax.set_title(title, fontsize=20, pad=10)
    ax.set_xlabel('填补月份数量', fontsize=20)
    ax.set_ylabel(name, fontsize=18)

    ax.set_xticklabels(['1', '3', '5'], fontsize=17)
    ax.tick_params(axis='y', labelsize=15)

    # ❗删除所有子图 legend
    if ax.get_legend():
        ax.get_legend().remove()

# ==================== 全局图例（关键新增） ====================
legend_handles = []
for j, label in enumerate(methods_order):
    color = PALETTE[j % len(PALETTE)]

    if label == 'OURS':
        patch = Rectangle((0, 0), 1.2, 1.2,
                          facecolor=color,
                          edgecolor='black',
                          hatch=hatch_pattern,
                          linewidth=1.5)
    else:
        patch = Rectangle((0, 0), 1.2, 1.2,
                          facecolor=color,
                          edgecolor='black',
                          linewidth=1.5)

    legend_handles.append(patch)

legend = fig.legend(
    legend_handles,
    methods_order,
    title='合成方法',
    loc='upper center',
    bbox_to_anchor=(0.5, 1.02),
    ncol=6,
    fontsize=17,
    title_fontsize=19,
    frameon=True
)

# OURS 加粗
for text in legend.get_texts():
    if text.get_text() == 'OURS':
        text.set_fontweight('bold')

# 给 legend 留空间
plt.tight_layout(rect=[0, 0, 1, 0.95])

plt.savefig('performance_comparison_CH.pdf', bbox_inches='tight', dpi=600)
plt.savefig('performance_comparison_CH.png', bbox_inches='tight', dpi=600)

print("已保存柱状图（全局顶部图例版本）")