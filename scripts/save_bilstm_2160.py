...
class BiLSTMModel(nn.Module):
    def __init__(self, input_size, hidden_size=64):
        ...
        self.fc1 = nn.Linear(hidden_size * 2, 32)  # 双向LSTM输出维度是hidden_size*2
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(32, 1)

    def forward(self, x):
        out, _ = self.bilstm(x)  # 输出形状: (batch_size, seq_len, hidden_size*2)
        out = out[:, -1, :]  # 取最后一个时间步
        out = self.fc1(out)
        out = self.relu(out)
        out = self.fc2(out)
...
# 保存训练曲线
def save_training_curve(history, building_name, sparsity, method, output_dir='results/bilstm/loss'):
    ...
    if len(parts) >= 4:
        ...
        if length == 2160 and sparsity in sparsity_rates:
            ...
            for method in methods:
                ...
                # 初始化模型、损失函数和优化器（使用BiLSTMModel）
                model = BiLSTMModel(input_size=6).to(device)
...
# 保存所有结果到总 CSV
if results_all:
    output_dir = '../results/bilstm_2160'  # 修改输出目录名
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
